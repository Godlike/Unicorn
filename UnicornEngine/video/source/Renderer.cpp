...
bool Renderer::CreateLogicalDevice()
{
	...
	createInfo.setEnabledExtensionCount(static_cast<uint32_t>(VulkanContext::deviceExtensions.size()));
	createInfo.setPpEnabledExtensionNames(VulkanContext::deviceExtensions.data());

	if (s_enableValidationLayers)
	{
		createInfo.setEnabledLayerCount(static_cast<uint32_t>(VulkanContext::validationLayers.size()));
		createInfo.setPpEnabledLayerNames(VulkanContext::validationLayers.data());
	}
	else
	{
...
bool Renderer::CheckDeviceExtensionSupport(const vk::PhysicalDevice& device)
{
	...
	auto deviceExtensions = VulkanContext::deviceExtensions;
