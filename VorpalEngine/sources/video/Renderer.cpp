...
namespace video
{
    ...
    bool Renderer::CreateSwapChain()
    {
        ...
        createInfo.imageUsage
            = VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT; // VK_IMAGE_USAGE_TRANSFER_DST_BIT for post processing.
    ...
    bool Renderer::CreateGraphicsPipeline()
    {
        ...
        bool shadersCreatedFailed
            = !CreateShaderModule(simpleVertShaderHandler.GetContent().GetBuffer(), vertShaderModule)
        ...
        if (vkCreateGraphicsPipelines(m_vkLogicalDevice, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &m_graphicsPipeline)
            != VK_SUCCESS)
        {
    ...
    bool Renderer::CreateFramebuffers()
    {
        ...
        for (size_t i = 0; i < m_swapChainImageViews.size(); i++)
        {
            ...
            if (vkCreateFramebuffer(m_vkLogicalDevice, &framebufferInfo, nullptr, &m_swapChainFramebuffers[i])
                != VK_SUCCESS)
            {
