...
Renderer::Renderer()
    : m_isInitialized(false), m_pWindow(nullptr), m_vkInstance(VK_NULL_HANDLE),
...
bool Renderer::Init() {
  ...
  m_pWindow = glfwCreateWindow(
      ...
      settings.GetApplicationName().c_str(), nullptr, nullptr);
  ...
  if (!CreateInstance() || !SetupDebugCallback() || !CreateSurface() ||
      ...
      !CreateIndexBuffer() || !CreateUniformBuffer() ||
      !CreateCommandBuffers() || !CreateSemaphores())
    return false;
  m_timer.Start();
...
void Renderer::Deinit() {
  ...
  if (m_descriptorSetLayout != VK_NULL_HANDLE) {
    vkDestroyDescriptorSetLayout(m_vkLogicalDevice, m_descriptorSetLayout,
                                 nullptr);
    m_descriptorSetLayout = VK_NULL_HANDLE;
  }
...
QueueFamilyIndices Renderer::FindQueueFamilies(VkPhysicalDevice device) {
  ...
  for (const auto &queueFamily : queueFamilies) {
    ...
    vkGetPhysicalDeviceSurfaceSupportKHR(device, i, m_vkWindowSurface,
                                         &presentSupport);
...
Renderer::QuerySwapChainSupport(VkPhysicalDevice device) {
  ...
  vkGetPhysicalDeviceSurfaceCapabilitiesKHR(device, m_vkWindowSurface,
                                            &details.capabilities);

  uint32_t formatCount;
  vkGetPhysicalDeviceSurfaceFormatsKHR(device, m_vkWindowSurface, &formatCount,
                                       nullptr);

  if (formatCount != 0) {
    details.formats.resize(formatCount);
    vkGetPhysicalDeviceSurfaceFormatsKHR(device, m_vkWindowSurface,
                                         &formatCount, details.formats.data());
  }

  uint32_t presentModeCount;
  vkGetPhysicalDeviceSurfacePresentModesKHR(device, m_vkWindowSurface,
...
bool Renderer::FindMemoryType(uint32_t typeFilter,
...
void Renderer::waitAsyncEnd() { vkDeviceWaitIdle(m_vkLogicalDevice); }
...
bool Renderer::CreateLogicalDevice() {
  ...
  if (vkCreateDevice(m_vkPhysicalDevice, &createInfo, nullptr,
                     &m_vkLogicalDevice) != VK_SUCCESS) {
    LOG_ERROR("Can't initialize Vulkan logical device!");
    return false;
  }
  vkGetDeviceQueue(m_vkLogicalDevice, indices.graphicsFamily, 0,
                   &m_graphicsQueue);
  vkGetDeviceQueue(m_vkLogicalDevice, indices.presentFamily, 0,
                   &m_presentQueue);
...
bool Renderer::CreateSurface() {
  if (glfwCreateWindowSurface(m_vkInstance, m_pWindow, nullptr,
                              &m_vkWindowSurface) != VK_SUCCESS) {
...
bool Renderer::CreateSwapChain() {
  ...
  if (vkCreateSwapchainKHR(m_vkLogicalDevice, &createInfo, nullptr,
  ...
  vkGetSwapchainImagesKHR(m_vkLogicalDevice, m_vkSwapChain, &imageCount,
                          nullptr);
...
bool Renderer::CreateImageViews() {
  ...
  for (uint32_t i = 0; i < m_swapChainImages.size(); i++) {
    ...
    if (vkCreateImageView(m_vkLogicalDevice, &createInfo, nullptr,
...
bool Renderer::CreateRenderPass() {
  ...
  if (vkCreateRenderPass(m_vkLogicalDevice, &renderPassInfo, nullptr,
...
bool Renderer::CreateGraphicsPipeline() {
  ...
  bool shadersCreatedFailed =
      !CreateShaderModule(simpleVertShaderHandler.GetContent().GetBuffer(),
                          vertShaderModule) ||
      !CreateShaderModule(simpleFragShaderHandler.GetContent().GetBuffer(),
                          fragShaderModule);
  ...
  if (vkCreateGraphicsPipelines(m_vkLogicalDevice, VK_NULL_HANDLE, 1,
...
bool Renderer::CreateFramebuffers() {
  ...
  for (size_t i = 0; i < m_swapChainImageViews.size(); i++) {
    ...
    if (vkCreateFramebuffer(m_vkLogicalDevice, &framebufferInfo, nullptr,
...
bool Renderer::CreateCommandPool() {
  ...
  if (vkCreateCommandPool(m_vkLogicalDevice, &poolInfo, nullptr,
...
bool Renderer::CreateCommandBuffers() {
  if (m_commandBuffers.size() > 0) {
    vkFreeCommandBuffers(m_vkLogicalDevice, m_commandPool,
  ...
  if (vkAllocateCommandBuffers(m_vkLogicalDevice, &allocInfo,
  ...
  for (size_t i = 0; i < m_commandBuffers.size(); i++) {
    ...
    vkCmdBindIndexBuffer(
        m_commandBuffers[i], m_indexBuffer, 0,
...
bool Renderer::CreateSemaphores() {
  ...
  if (vkCreateSemaphore(m_vkLogicalDevice, &semaphoreInfo, nullptr,
                        &m_imageAvailableSemaphore) != VK_SUCCESS ||
      vkCreateSemaphore(m_vkLogicalDevice, &semaphoreInfo, nullptr,
...
bool Renderer::CreateUniformBuffer() {
  ...
  if (!CreateBuffer(bufferSize,
                    ...
                    VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, m_uniformBuffer,
...
void Renderer::UpdateUniformBuffer() {
  ...
  void *data;
  vkMapMemory(m_vkLogicalDevice, m_uniformStagingBufferMemory, 0, sizeof(ubo),
              0, &data);
...
bool Renderer::CreateDescriptorSetLayout() {
  ...
  if (vkCreateDescriptorSetLayout(m_vkLogicalDevice, &layoutInfo, nullptr,
...
bool Renderer::CreateShaderModule(const std::vector<uint8_t> &code,
                                  VkShaderModule &shaderModule) {
  VkShaderModuleCreateInfo createInfo = {};
  createInfo.sType = VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO;
  createInfo.codeSize = code.size();
  createInfo.pCode = (uint32_t *)code.data();
  if (vkCreateShaderModule(m_vkLogicalDevice, &createInfo, nullptr,
...
bool Renderer::CreateBuffer(VkDeviceSize size, VkBufferUsageFlags usage,
  ...
  if (vkAllocateMemory(m_vkLogicalDevice, &allocInfo, nullptr, &bufferMemory) !=
      VK_SUCCESS) {
...
bool Renderer::CopyBuffer(VkBuffer srcBuffer, VkBuffer dstBuffer,
                          VkDeviceSize size) {
  VkCommandBufferAllocateInfo allocInfo = {};
  allocInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO;
  allocInfo.level = VK_COMMAND_BUFFER_LEVEL_PRIMARY;
...
bool Renderer::Frame() {
  ...
  VkResult result = vkAcquireNextImageKHR(
      m_vkLogicalDevice, m_vkSwapChain, std::numeric_limits<uint64_t>::max(),
